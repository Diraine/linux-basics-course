...
1. To run commands that need **`sudo`**(super-user) privilages. Run **`sudo`**
```
$ sudo ls /root
```

2. To check the **`init process`** (systemd or sysV) used by the system
```
$ sudo ls -l /sbin/init
```

3. To check **`default systemd target`** (eg. graphical.target or multi-user.target) set in the system
```
$ sudo systemctl get-default
```

4. To change the systemd target to **`multi-user.target`**
```
...
6. To check what type of file is **`sample_script.sh`** which is located at /root
```
$ sudo file /root/sample_script.sh
```

7. You were asked to install a new **`third-party IDE`** in the system. Which directory  is the recommended choice for the installation?
```
